...
### for reference when not using a session
...
if ( $vAppTrunks.count -gt 0 ) {
	...
	# Collect all the vswitches and portgroups to search for an unused vlan 
    $vSwitches=get-cluster | ?{ $_.Name -eq $conf.VICluster } | get-vmhost | get-virtualswitch | ?{ $_.Name -eq $conf.vswitch}
    $portGroups=$vSwitches | get-virtualportgroup
    [int]$pgVLan=$conf.vlanbase

	# create new portgroups with unique VLAN IDs
	write-host "Creating portGroups:"
    foreach($pg in $( $srcApp | get-vm | get-virtualportgroup | where { $_.Name -like "$vApp*" })) {
        ...
        # Create portgroup lab_%name%_%suffix% on all hosts
        $pgName="$vAppNew"+$pg.name.substring($vApp.length)
        Write-Host ":: $pg => $pgName"
        $result=$vSwitches | new-virtualportgroup -name $pgName -vlanid $pgVLan -ErrorAction SilentlyContinue
    }
}
...
# Connect LAN nics to the new portgroups
write-host "Connecting LAN Nics"
$cloneVMs=$cloneApp | get-vm
#$portGroups=$cloneVMs | get-virtualportgroup | where { $_.Name -like "$vApp*" }
$portGroups=$cloneVMs | ?{ $_.Name -notlike "*gateway*" } | get-virtualportgroup
$networkAdapters=$cloneVMs | get-networkadapter

foreach($pg in $portGroups){
    # pg.name is in format $vApp_pgName
    if( $pg.Name -like "$vApp*" ){ $pgName = $vAppNew+$pg.Name.substring($vApp.Length)}
    #otherwise its just pgName
    else { $pgName = $vAppNew+"_"+$pg.Name }

    write-host ":: $pg => $pgName"
    $result=$networkAdapters | where {$_.NetworkName -eq $pg } | set-networkadapter -NetworkName "$pgName" -confirm:$false
}
