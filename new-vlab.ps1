...
# Parameters Override Conf
# Pick the host with the most free ram
if ( ! $conf.vmwHost ){
...
# Feedback
Write-Host "Provisioning vLab: $vAppNew from: $vApp on:"$conf.vmwHost

# FlexClone the vApp volume
Write-Host "Creating Volume FlexClone /$vApp/$vAppNew"
$result=New-NcVolClone $vAppNew $vApp -JunctionPath "/$vApp/$vAppNew" -ParentSnapshot master

# Create New vApp
write-host "Creating vApp $vappnew"
$result=New-VApp -Name $vAppNew -Location (Get-Cluster $conf.vmwCluster)

# Creating a staging folder
#write-host "Creating a vApp staging folder"
#$viFolder=new-folder -name $vAppNew -location (get-datacenter)

# Create New portgroups
# vlans are internal in the range 1000-1999
# Hosts have a portgroup limit of 500ish which would be hit first
write-host "Creating portgroups"
$pgID=1
[int]$pgVLan=$conf.vlanbase
$virtualSwitches=get-cluster | ?{ $_.Name -eq $conf.vmwCluster } | get-vmhost | get-virtualswitch | ?{ $_.Name -eq $conf.vswitch}
...
foreach($srcPortGroup in $(get-vapp $vApp | get-vm | get-virtualportgroup | where { $_.Name -like "$vApp*" })) {
	...
	DO {
		$pgVLan++
		#$result=get-virtualswitch -host $conf.vmwHost -name $conf.vswitch | get-virtualportgroup | where { $_.VLanID -eq $pgVLan } 
		#$result=get-cluster | ?{ $_.Name -eq $conf.vmwCluster } | get-vmhost | get-virtualswitch | ?{ $_.Name -eq $conf.vswitch} | get-virtualportgroup | where { $_.VLanID -eq $pgVLan }
	...
	#$result=get-virtualswitch -host $conf.vmwHost -name $conf.vswitch | new-virtualportgroup -name $pgName -vlanid $pgVLan
#	$result=get-cluster | ?{ $_.Name -eq $conf.vmwCluster } | get-vmhost | get-virtualswitch | ?{ $_.Name -eq $conf.vswitch} | new-virtualportgroup -name $pgName -vlanid $pgVLan
...
foreach($VMXFolder in $SearchResult) {
	foreach($VMXFile in $VMXFolder.File) {
		...
		$VM=New-VM -VMFilePath $vmx -VMHost $ESXHost -ResourcePool $vAppNew 
...
foreach($srcPortGroup in $(get-vapp $vApp | get-vm | get-virtualportgroup | where { $_.Name -like "$vApp*" })) {
	$pgName="$vAppNew"+"_$pgID"
	#$result=get-vapp $vAppNew | get-vm | get-networkadapter | where {$_.NetworkName -eq $srcPortGroup } | set-networkadapter -NetworkName "$pgName" -confirm:$false
...
foreach($srcPortGroup in $(get-vapp $vApp | get-vm | get-virtualportgroup | where { $_.Name -eq $oldWAN })) {
	$result=get-vapp $vAppNew | get-vm | get-networkadapter | where {$_.NetworkName -eq $srcPortGroup } | set-networkadapter -NetworkName $conf.pgDMZ -confirm:$false
}
...
$result=New-DrsRule -Cluster $conf.vmwCluster -Name $vAppNew -KeepTogether $true -VM (get-vapp $vAppNew | get-vm)
