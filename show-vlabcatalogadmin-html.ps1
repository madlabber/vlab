...
$result=invoke-command -session $session -scriptblock { 
    ...
    # $output+="      <input type=button value=Start onclick=`"window.open('$starturl')`"/>"
      ...
      $output+="      <button type=`"submit`" formaction=`"/authoron?$($instance.name)`">Enable Authoring</button>"
      $output+="      <button type=`"submit`" formaction=`"/authoroff?$($instance.name)`">Disable Authoring</button>"
